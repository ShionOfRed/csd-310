...
-- Create STORE, BOOK, USER, and WISHLIST tables --
CREATE TABLE store (
...
INSERT INTO wishlist(user_id, book_id)
    VALUES (
        (SELECT user_id FROM user WHERE first_name = 'Frodo' LIMIT 1),
        (SELECT book_id FROM book WHERE book_name = 'The Return of the King' LIMIT)
    );
